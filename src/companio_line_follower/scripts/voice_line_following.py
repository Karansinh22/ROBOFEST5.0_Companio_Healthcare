#!/usr/bin/env python3
"""
Voice-Controlled Line Following with Location Detection

Features:
- Follows a single open-loop line
- Detects 4 location patches (where 4-5 IR sensors read BLACK)
- Voice commands: location 1/2/3/4, home, stop, forward, backward, continue, over/quit
...
try:
    ...
    print(f"ERROR: speech_recognition not found. Install with: pip3 install SpeechRecognition")
    print(f"Error: {e}")
    sys.exit(1)


class VoiceLineFollower:
    """Line follower with location detection and voice control."""
    ...
    def __init__(self) -> None:
        """Initialize the voice-controlled line follower."""
        rospy.init_node("voice_line_following", anonymous=False)
        ...
        self.rotation_time = float(rospy.get_param("~rotation_time", 5.0))  # seconds
        ...
        self.record_duration = int(rospy.get_param("~record_duration", 3))  # Integer for arecord (seconds)
        self.audio_file = '/tmp/voice_line_following.wav'
        self.voice_listen_rate = float(rospy.get_param("~voice_listen_rate", 0.33))  # ~3 seconds per cycle
        ...
        rospy.loginfo("=" * 60)
        rospy.loginfo("Voice Line Follower initialized")
        rospy.loginfo("=" * 60)
        rospy.loginfo("Voice Commands:")
        rospy.loginfo("  location 1, 2, 3, 4  → Go to location")
        rospy.loginfo("  home                 → Go Home")
        rospy.loginfo("  stop                 → Stop")
        rospy.loginfo("  forward              → Force forward")
        rospy.loginfo("  backward             → Force backward")
        rospy.loginfo("  continue             → Continue previous task")
        rospy.loginfo("  over / quit          → Quit")
        rospy.loginfo("=" * 60)
    ...
    def record_audio(self) -> bool:
        ...
        try:
            if self.debug:
                rospy.logdebug("🎤 Recording... SPEAK NOW!")
            
            # Record using arecord (duration must be integer, not float)
            duration_int = int(self.record_duration)  # Ensure integer
            result = subprocess.run([
                ...
                '-c', '1',
                '-d', str(duration_int),  # Use integer
                self.audio_file
            ], capture_output=True, text=True)
            
            if result.returncode == 0:
                if self.debug:
                    rospy.logdebug("✓ Recording complete!")
                return True
            else:
                rospy.logwarn_throttle(5.0, f"Recording failed: {result.stderr}")
                return False
                
        except Exception as e:
            rospy.logwarn_throttle(5.0, f"Error recording audio: {e}")
            return False
    ...
    def recognize_command(self) -> Optional[str]:
        ...
        try:
            ...
            if not os.path.exists(self.audio_file):
                rospy.logwarn_throttle(5.0, "Audio file not found!")
                return None
            ...
            rospy.logwarn_throttle(5.0, "✗ Could not understand audio")
            return None
        except sr.RequestError as e:
            rospy.logwarn_throttle(5.0, f"✗ Speech recognition error: {e}")
            return None
        except Exception as e:
            rospy.logwarn_throttle(5.0, f"✗ Error: {e}")
            return None
    
    def _process_voice_command(self, command: str) -> bool:
        """
        Process voice command and update state.
        ...
        if 'over' in command_lower or 'exit' in command_lower or 'quit' in command_lower:
            rospy.loginfo("🛑 QUIT command received. Shutting down...")
            self._publish_stop()
            self._shutdown_requested = True
            return False  # Stop running
        
        # Location commands - handle both numbers and words
        if 'location' in command_lower:
            ...
            # Extract location number (check for words first, then digits)
            target = None
            ...
            if target is None:
                rospy.logwarn(f"Could not extract location number from: '{command}'")
                return True  # Continue running
        ...
        # Stop command
        elif 'stop' in command_lower or 'halt' in command_lower or 'wait' in command_lower:
        ...
        # Continue command
        elif 'continue' in command_lower or 'resume' in command_lower:
            ...
            rospy.logwarn_throttle(5.0, f"❓ Unknown command: '{command}'")
            return True  # Continue running
    ...
    def main_loop(self) -> None:
        ...
        try:
            while not rospy.is_shutdown() and not self._shutdown_requested:
                ...
                # Check for voice input ONLY when not actively following line
                # Voice recognition blocks for 3 seconds, so we skip it during navigation
                if self.state in [self.State.IDLE, self.State.STOPPED, self.State.MANUAL_OVERRIDE]:
                    if (now - self.last_voice_check_time) >= (1.0 / self.voice_listen_rate):
                        self.last_voice_check_time = now
                        command = self.recognize_command()
                        if command:
                            if not self._process_voice_command(command):
                                break  # Quit command received
                # During LINE_FOLLOW or ROTATING, skip voice recognition to avoid blocking
                # User can say "stop" but it will only be processed after navigation completes
                ...
                if self.state == self.State.IDLE:
                        ...
                        if (now - self.location_detection_start_time) >= self.location_debounce_time:
                            ...
                            if not self.already_detected:
                                ...
                                if is_home and self.target_location == 0:
                                    ...
                                    rospy.loginfo("Ready for next voice command...")
                                    rospy.loginfo("=" * 60)
                                    self._publish_stop()
                                    self.state = self.State.STOPPED
                                    self.target_location = None
                                    # Reset detection flags for next command
                                    self.already_detected = False
                                    self.location_detection_start_time = 0.0
                                    continue
                    ...
                    # User can give forward, backward, stop, or continue commands
                    # Velocity is set in _process_voice_command
                    pass
            ...
            # Cleanup on shutdown
            self._publish_stop()
            rospy.loginfo("Voice line follower shutting down. Cleaning up...")
            try:
                try:
                    self._lines.release()
                except Exception:
...
def main() -> None:
    ...
    node = None
    try:
        node = VoiceLineFollower()
    except rospy.ROSInterruptException:
        pass
    except KeyboardInterrupt:
        rospy.loginfo("Keyboard interrupt received")
    except Exception as exc:
        rospy.logfatal("voice_line_following crashed: %s", exc)
        import traceback
        traceback.print_exc()
        time.sleep(0.5)
    finally:
