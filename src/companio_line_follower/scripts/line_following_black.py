...
This version moves when sensors detect BLACK (low reflection/no light).
"""
...
class LineFollowerGPIO:
    def __init__(self) -> None:
        ...
        try:
            ...
            self._lines.request(
                consumer="line_follower_gpio_black",
    ...
    def _compute_twist(self, sensors: List[int]) -> Twist:
        ...
        """
        # Count sensors seeing the black line (value 0 = no reflection)
        active = sum(1 for s in sensors if s == 0)
        cmd = Twist()

        if active == 0:
            # No sensor sees the black line → stop
        ...
        line_bits = [1 - s for s in sensors]  # 0 becomes 1, 1 becomes 0
...
def main() -> None:
    rospy.init_node("line_follower_gpio_black", anonymous=False)
    rospy.loginfo("Starting line_follower_gpio_black node...")
    ...
    try:
        ...
        rospy.logfatal("line_follower_gpio_black crashed: %s", exc)
