#!/usr/bin/env python3
"""
Companio Left Arm Sequence Controller
...
# Arm: [10, 150]
# Elbow: [15, 185]
# Gripper: [30, 175] (175 is closed)

H_SEQUENCE = [
    [85.0, 85.0, 60.0, 115.0],  # Stage 1
    [145.0, 80.0, 60.0, 115.0], # Stage 2
    [135.0, 85.0, 75.0, 115.0], # Stage 3
    [135.0, 85.0, 75.0, 170.0], # Stage 4
    [80.0, 85.0, 95.0, 170.0]   # Stage 5
]
...
def main():
    rospy.init_node('left_arm_sequence_controller')
    pub = rospy.Publisher('/left_arm/joint_commands', Float64MultiArray, queue_size=1)
    
    # Subscribe to vision system
    rospy.Subscriber('/gripper_vision/left/target_detected', Bool, vision_callback)
    
    # Mutual exclusion topics
    status_pub = rospy.Publisher('/left_arm/is_moving', Bool, queue_size=1, latch=True)
    status_pub.publish(False)
    rospy.Subscriber('/right_arm/is_moving', Bool, status_callback)
    
    # Initialize tracking - starting from home
    current_pos = [80.0, 85.0, 95.0, 160.0]
    last_pick_time = 0.0
    ...
    while not rospy.is_shutdown():
        ...
        if key == 'H':
            if other_arm_moving:
                print(">>> WARNING: Cannot move. Right arm is currently active. <<<")
                prompt_printed = False
                continue
            ...
            # Step 1: Open gripper to 115
            open_pos = [current_pos[0], current_pos[1], current_pos[2], 115.0]
            wait_open = calculate_wait_time(open_pos, current_pos)
            
            print(f"  Opening gripper to 115... (Wait {wait_open:.2f}s)")
            msg = Float64MultiArray()
            msg.data = open_pos
            pub.publish(msg)
